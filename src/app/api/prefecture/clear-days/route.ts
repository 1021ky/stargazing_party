...
const PUBLIC_ALLOWED_MIN =
  process.env.NEXT_PUBLIC_OPEN_METEO_ALLOWED_START_DATE_MIN;
const PUBLIC_ALLOWED_MAX =
  process.env.NEXT_PUBLIC_OPEN_METEO_ALLOWED_START_DATE_MAX;
const SERVER_ALLOWED_MIN =
  process.env.OPEN_METEO_ALLOWED_START_DATE_MIN ?? PUBLIC_ALLOWED_MIN;
const SERVER_ALLOWED_MAX =
  process.env.OPEN_METEO_ALLOWED_START_DATE_MAX ?? PUBLIC_ALLOWED_MAX;
...
function computeWindowBounds(): {
  ...
  const start = SERVER_ALLOWED_MIN
    ? maxIso(todayIso, SERVER_ALLOWED_MIN)
    : todayIso;
  const tentativeEnd = addDaysUtcIso(start, WINDOW_DAYS - 1);
  const end = SERVER_ALLOWED_MAX
    ? minIso(tentativeEnd, SERVER_ALLOWED_MAX)
    : tentativeEnd;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!coords) {
      return NextResponse.json(
        { message: `Unsupported prefecture: ${prefecture}` },
        { status: 400 },
      );
    }
    ...
    const summaries = await getDailyWeatherSummariesRange(
      coords.latitude,
      coords.longitude,
      start,
      end,
