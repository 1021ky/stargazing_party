...
export async function getBlackMarbleProxy({
  ...
  const label = `[DEBUG:BlackMarble] lat=${latitude.toFixed(4)} lon=${longitude.toFixed(4)} year=${year}`;

  const endpoint = process.env.BLACK_MARBLE_POINT_QUERY_ENDPOINT;
  if (!endpoint) {
    console.log(`${label} - ENDPOINT NOT SET`);
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Black Marble point query failed with status ${response.status}`,
    );
  }
  ...
  const proxyIsNoData =
    proxyRaw === BLACK_MARBLE_FILL_VALUE || proxyRaw === null;
  const qualityIsNoData = qualityFlag === BLACK_MARBLE_NO_DATA_QUALITY;

  console.log(
    `${label} - Response: proxyRaw=${proxyRaw} qualityFlag=${qualityFlag} proxyIsNoData=${proxyIsNoData} qualityIsNoData=${qualityIsNoData}`,
  );
