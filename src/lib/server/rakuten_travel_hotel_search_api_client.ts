...
function createFetchClient(): Fetcher {
  return async (input, init = {}) => {
        ...
        .then(async (response) => {
          ...
          if (response.status === 400) {
            try {
              ...
              console.error(
                "Rakuten Travel API returned 400. Response body:",
                bodyText,
              );
            } catch (error) {
              console.error(
                "Rakuten Travel API returned 400 but response body could not be read.",
                error,
              );
            }
          }
    ...
    throw (
      lastError ??
      new Error("Failed to fetch Rakuten Travel Simple Hotel Search API")
    );
  };
}
...
function transformHotelsToAccommodation(
  payload: unknown,
  stayDate: string,
  ...
  const { error, error_description: errorDescription } =
    payload as RakutenApiResponseError;
  if (error) {
    throw new Error(errorDescription ?? error);
  }

  const hotels = Array.isArray(
    (payload as { hotels?: RakutenHotelWrapper[] }).hotels,
  )
    ? ((payload as { hotels?: RakutenHotelWrapper[] })
        .hotels as RakutenHotelWrapper[])
    : [];
...
function extractAccommodationFromWrapper(
  wrapper: RakutenHotelWrapper,
  stayDate: string,
): RakutenHotelAccommodation | null {
  const hotelEntries = Array.isArray(wrapper.hotel) ? wrapper.hotel : [];

  const basicInfo =
    hotelEntries.find((entry) => entry.hotelBasicInfo)?.hotelBasicInfo ??
    wrapper.hotelBasicInfo ??
    undefined;
  const detailInfo =
    hotelEntries.find((entry) => entry.hotelDetailInfo)?.hotelDetailInfo ??
    wrapper.hotelDetailInfo ??
    undefined;
  const ratingInfo =
    hotelEntries.find((entry) => entry.hotelRatingInfo)?.hotelRatingInfo ??
  ...
  const rating = normaliseNumber(
    basicInfo.reviewAverage ?? ratingInfo?.totalScore ?? 0,
    1,
  );
  const clearSkyProbability = estimateClearSkyProbability(
    rating,
    ratingInfo?.locationAverage,
  );
  const availableRooms =
    detailInfo?.roomCount && detailInfo.roomCount > 0
      ? detailInfo.roomCount
      : 1;

  const latitude =
    typeof basicInfo.latitude === "number" ? basicInfo.latitude : null;
  const longitude =
    typeof basicInfo.longitude === "number" ? basicInfo.longitude : null;
  const altitude =
    latitude !== null ? estimateAltitudeFromLatitude(latitude) : 0;
...
function mergeAccommodations(
  accommodations: RakutenHotelAccommodation[],
): RakutenHotelAccommodation[] {
  ...
  for (const accommodation of accommodations) {
    ...
    const combinedAvailableRooms =
      existing.availableRooms + accommodation.availableRooms;
    const betterRating =
      accommodation.rating > existing.rating
        ? accommodation.rating
        : existing.rating;
    const betterClearSky = Math.max(
      existing.clearSkyProbability,
      accommodation.clearSkyProbability,
    );
    const newMoonDate =
      existing.newMoonDate <= accommodation.newMoonDate
        ? existing.newMoonDate
        : accommodation.newMoonDate;
...
function normaliseStayDates(stayDates: Array<string | Date>): string[] {
  ...
  return stayDates
    .map((date) => normaliseDateInput(date))
...
function normaliseDateInput(value: string | Date): string {
  ...
  if (typeof value === "string") {
    ...
    if (Number.isNaN(parsed.getTime())) {
      throw new TypeError(
        "stayDates must only contain ISO 8601 date strings (YYYY-MM-DD) or Date objects",
      );
    }
...
function extractPrefectureAndCity(address: string): {
  prefecture: string;
  city: string;
} {
...
function estimateClearSkyProbability(
  reviewAverage: number,
  locationAverage?: number,
): number {
  const base = Number.isFinite(locationAverage)
    ? (locationAverage ?? 0)
    : reviewAverage;
...
function formatNextNewMoonDate(referenceDate = new Date()): string {
  ...
  const diffDays =
    (referenceDate.getTime() - referenceNewMoon.getTime()) / msPerDay;
  const cycles = Number.isFinite(diffDays)
    ? Math.max(0, Math.ceil(diffDays / SYNODIC_MONTH_DAYS))
    : 0;
  const nextNewMoon = new Date(
    referenceNewMoon.getTime() + cycles * SYNODIC_MONTH_DAYS * msPerDay,
  );
...
function buildRakutenBookingUrl(
