...
describe("fetchGibsPixelBrightness", () => {
  ...
  it.each([
    ...
    "$label ピクセルの平均輝度を返す",
    async ({ r, g, b, expectedBrightness }) => {
  ...
  it("WMS リクエストに正しいパラメータが含まれる", async () => {
    ...
    await fetchGibsPixelBrightness(34.6, 135.7);
  ...
  it("month 指定時は TIME パラメータが YYYY-MM-01 形式になる", async () => {
    const pngBuffer = buildTestPng(0, 0, 0);
    const mockFetch = jest.spyOn(global, "fetch").mockResolvedValue(
      new Response(pngBuffer, {
        status: 200,
    ...
    await fetchGibsPixelBrightness(34.6, 135.7, 2023, 6);

    const calledUrl = mockFetch.mock.calls[0][0] as string;
    const url = new URL(calledUrl);
    expect(url.searchParams.get("TIME")).toBe("2023-06-01");
  });

  it("month 省略時は TIME パラメータが YYYY-01-01 になる", async () => {
    const pngBuffer = buildTestPng(0, 0, 0);
    const mockFetch = jest.spyOn(global, "fetch").mockResolvedValue(
      new Response(pngBuffer, {
        status: 200,
    ...
    await fetchGibsPixelBrightness(34.6, 135.7, 2023);

    const calledUrl = mockFetch.mock.calls[0][0] as string;
    const url = new URL(calledUrl);
    expect(url.searchParams.get("TIME")).toBe("2023-01-01");
  });
