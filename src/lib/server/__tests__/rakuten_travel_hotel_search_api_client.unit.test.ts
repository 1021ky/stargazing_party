...
describe("searchHotelsWithAvailability (unit)", () => {
  ...
  it("APIが200を返した場合に宿泊データを返し、複数日の結果をマージする", async () => {
    ...
    const accommodations = await searchHotelsWithAvailability(
      latitude,
      longitude,
      stayDates,
    );
  ...
  it("環境変数が設定されていない場合は例外を投げる", async () => {
    ...
    await expect(
      searchHotelsWithAvailability(latitude, longitude, ["2025-02-01"]),
    ).rejects.toThrow("RAKUTEN_APP_ID is not set");
  });

  it("宿泊日が空配列の場合は例外を投げる", async () => {
    await expect(
      searchHotelsWithAvailability(latitude, longitude, []),
    ).rejects.toThrow("stayDates must contain at least one date");
  });
});
