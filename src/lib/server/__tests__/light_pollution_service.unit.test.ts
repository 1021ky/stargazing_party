...
describe('resolveLightPollution', () => {
    const { getBlackMarbleProxy } = jest.requireMock('../black_marble_api_client');
    const { fetchGibsPixelBrightness } = jest.requireMock('../gibs_light_pollution_client');
    ...
    ])('proxyRaw=$proxyRaw の境界値で level=$level を返す', async ({ proxyRaw, level }) => {
    ...
    it('BLACK_MARBLE 例外発生時に GIBS が成功した場合は gibs-black-marble-2016 を返す', async () => {
    ...
    it.each([
    ...
    ])('GIBS brightness=$brightness の境界値で level=$level を返す', async ({ brightness, level }) => {
        ...
        expect(result.lightPollutionLevel).toBe(level);
        expect(result.lightPollutionSource).toBe('gibs-black-marble-2016');
    });
