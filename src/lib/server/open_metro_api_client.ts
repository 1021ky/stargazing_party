...
const OPEN_METEO_ALLOWED_START_DATE_MIN =
  process.env.OPEN_METEO_ALLOWED_START_DATE_MIN;
const OPEN_METEO_ALLOWED_START_DATE_MAX =
  process.env.OPEN_METEO_ALLOWED_START_DATE_MAX;
...
async function fetchWeatherWithRetry(
  ...
  while (attempt < MAX_FETCH_RETRIES) {
    try {
      ...
      const json = await fetchOpenMeteoByIp(
        params,
        RETRY_DELAY_MS * attempt + 10000,
      );
      ...
      if (attempt >= MAX_FETCH_RETRIES) {
        ...
        throw new Error(
          `Open-Meteo API へのアクセスに失敗しました (${message})`,
        );
      }
...
function revealAdapter(adapter: any) {
  ...
  const hourlyObj = hourly
    ? {
        startTimestamp:
          typeof hourly.time === "function" ? hourly.time() : hourly.time,
        intervalSeconds:
          typeof hourly.interval === "function"
            ? hourly.interval()
            : hourly.interval,
  ...
  return {
    ...
    daily: () => {
      ...
      return {
        ...
        interval: () => {
          ...
          return Math.floor(
            (new Date(t[1]).getTime() - new Date(t[0]).getTime()) / 1000,
          );
        },
      };
    },
    hourly: () => {
      ...
      return {
        ...
        isoTimes: () =>
          Array.isArray(hourly.time) ? Array.from(hourly.time) : [],
      };
    },
...
function extractSummariesFromResponse(
  ...
  const temperatureMaxValues = extractVariableValues(
    daily,
    1,
    "temperature_2m_max",
  );
  const temperatureMinValues = extractVariableValues(
    daily,
    2,
    "temperature_2m_min",
  );
  const length = Math.min(
    weatherCodes.length,
    temperatureMaxValues.length,
    temperatureMinValues.length,
  );
  ...
  for (let index = 0; index < length; index += 1) {
    ...
    const isClearSky =
      typeof hourlyClearSky === "boolean" ? hourlyClearSky : baseIsClearSky;
  ...
  if (!summaries.length) {
    throw new Error(
      "Open-Meteo API から対象期間のデータを取得できませんでした",
    );
  }
